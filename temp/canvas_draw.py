...
class App:
    ...
    def item_bind(self, t):
        # 为鼠标右键拖动事件绑定处理函数
        self.cv.tag_bind(t, '<B2-Motion>', self.move)
        # 为鼠标右键释放事件绑定处理函数
        self.cv.tag_bind(t, '<ButtonRelease-2>', self.move_end)
        ...
        if self.item_type == 0:
            ...
            if self.firstx > 0 and self.firsty > 0:
                ...
                t = self.cv.create_line(self.firstx, self.firsty,
                    event.x, event.y, fill=self.outline, width=self.width.get(), joinstyle=ROUND)
